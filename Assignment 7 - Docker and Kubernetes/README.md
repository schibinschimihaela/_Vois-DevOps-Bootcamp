...
! As shown in the screenshot, the NGINX application was exposed using both ClusterIP and NodePort services. The services are visible in the terminal, and the application is accessible from the browser via the NodePort.
...
CPU load was artificially generated by adding a configuration line that increased CPU consumption in the NGINX deployment. As a result, the Horizontal Pod Autoscaler detected the increased resource usage and scaled the deployment up to the maximum of five replicas in order to handle the load.

![HPA](Kubernetes/photos/5-3.png)

After removing the configuration that generated high CPU load on the NGINX containers, CPU utilization gradually decreased. Following the stabilization period, the Horizontal Pod Autoscaler scaled the deployment down to the minimum number of replicas after approximately five minutes.
...
A Pod crash was intentionally simulated by configuring a container to run the command `"echo Starting app; exit 1"`, which caused the container to terminate immediately. The `kubectl describe` and `kubectl logs` commands were then used to inspect the Pod’s behavior and identify the cause of the failure, confirming why the Pod entered a crash state.
...
To demonstrate how an IAM Role can be created and associated with a Kubernetes ServiceAccount
using IRSA, I created a `service-account.yml` file. This file illustrates how the IAM role ARN
is attached to a Kubernetes ServiceAccount through annotations.

Additionally, I created a `deployment-app.yml` file to show how this ServiceAccount is
referenced in a Deployment using the `serviceAccountName` field. This ensures that the Pods
running the application assume the permissions defined by the associated IAM role.

This configuration is presented conceptually, as the Kubernetes cluster used in this
...
connections to the Redis backend via its ClusterIP Service. This design ensures scalability, and resilience, as Pods can be replaced or scaled independently without impacting connectivity.
